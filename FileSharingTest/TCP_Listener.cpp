	...
	while (1) {
		...
		if (iResult > 0) {
			...
			if (!value.compare("2")) { //quando str == 2 L'host che ci ha contattato vuole la foto-> entra qui dentro quindi invio l'ack al Client che mi ha contattato e lancio un nuovo Client 
				...
				TCP_Client* newClient = new TCP_Client("", *Client_IP, 3, MAC, mutShared, sharedFotoPath, nullptr); //La task di questo Client è inviare una foto...prima contatterà il Listener dall'altra parte con valore=3 -> poi contatterà il server creato dal listener dall'altra parte
